    ...
    [
        dbc.Card(
            [
                ...
                dbc.Row(dcc.Graph(id="graph", figure=sales_pie()), justify="center"),
            ],
            className="mt-3",
        )
    ]
    ...
    if trigger_id == "graph":
        ...
        if click_data is not None:
            ...
            if vendor in product_sales_df.vendors.unique():
                ...
                vendor_sales_df = product_sales_df[
                    product_sales_df["vendors"] == vendor
                ]

                # generating product sales bar graph
                fig = px.bar(
                    vendor_sales_df, x="products", y="units sold", color="products"
                )
                fig.update_layout(
                    title="<b>{} product sales<b>".format(vendor),
                    showlegend=False,
